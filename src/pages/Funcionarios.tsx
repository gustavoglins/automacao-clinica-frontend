...
const Funcionarios = () => {
  ...
  const filteredEmployees = employees.filter((employee) => {
    ...
    const matchesSearch =
      ...
      employee.phone.includes(term) || // Busca no telefone formatado
      employee.phone.replace(/\D/g, "").includes(term.replace(/\D/g, "")); // Busca no telefone sem formatação
  ...
  const filteredDialogEmployees = filteredEmployees.filter(emp =>
    emp.name.toLowerCase().includes(dialogSearch.toLowerCase()) ||
    emp.role.toLowerCase().includes(dialogSearch.toLowerCase()) ||
    (emp.specialty ? emp.specialty.toLowerCase().includes(dialogSearch.toLowerCase()) : false) ||
    emp.email.toLowerCase().includes(dialogSearch.toLowerCase()) ||
    emp.phone.includes(dialogSearch) || // Busca no telefone formatado
    emp.phone.replace(/\D/g, "").includes(dialogSearch.replace(/\D/g, "")) // Busca no telefone sem formatação
  );
  ...
  function DialogSearchBar() {
    return (
      <div className="mb-4">
        <Input
          placeholder="Buscar funcionário por nome, função, especialidade, email ou telefone..."
          value={dialogSearch}
      ...
      <div className="space-y-6">
        ...
        <Dialog open={openAllEmployeesDialog} onOpenChange={setOpenAllEmployeesDialog}>
          <DialogContent className="max-w-5xl w-full">
            ...
            {/* Barra de pesquisa interna do Dialog */}
            <DialogSearchBar />
